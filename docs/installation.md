...
当前推荐使用节省磁盘模式：锅巴中关闭“启用下载缓存”、开启“发送后删除”。插件会等待 LLBot/Yunzai 的发送调用结束后再移除成品；挂载仍然必须保留，因为发送发生时 LLBot 需要读取同一路径。启动约60秒后和每天 `04:20` 的兜底任务只清理 `data/bilibili/`，不会触碰账号、签到或其他插件数据，并避开 `04:10` 的攻略刷新。完整参数见 [B站解析与下载](bilibili.md#下载文件缓存与清理策略)。

...
- `#全部补签`：主人对所有已启用的 Profile 立即执行一次签到，不受原计划时间和 `done` 状态限制。任务串行执行，每个 Profile 完成后立即返回一张与 `#补签` 相同的签到结果图，全部结束后再返回总汇图，因此耗时较长时也能持续看到成功或失败进度。结果写回今日计划，适合网络恢复后补做已经失败的任务。成功条目标记完成；`refresh`、runner、超时等可重试失败会从补签时刻重新按 `failure_retry_minutes` 安排自动重试，不会被错误标记为完成。

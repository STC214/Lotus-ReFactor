...
> 2026-08-25 后 Lotus 增加了三游戏攻略作者源兜底：原神整合“你的夏木繁 / Asgater / 猫冬”，星铁整合“让我摸个鱼吧Moyu / 祈鸢ya / 小橙子阿”，绝区零整合“Asgater / 丶ATRI丶 / 洗礼酱”。当 Auto-Plugin、miao-plugin 等其他插件没有处理重叠攻略指令时，才由 Lotus 返回 QQ 合并转发结果；每位命中作者一个节点，节点内包含作者、标题、米游社链接和攻略图。

Lotus 作者库保存在 `data/strategy-authors/cache.json`。每天两次的后端任务 `0 10 4,16 * * ? *` 和主人指令 `#更新攻略作者库` 使用增量检查，不再重复下载全部历史文章；用户查询采用12小时新鲜期，命中旧缓存时立即回复并在过期后后台并行刷新。当前 UTC 容器的定时任务对应北京时间 12:10 和次日 00:10。接口失败时保留本地旧缓存。完整算法、返回字段和验证命令见[三游戏攻略本地缓存与增量刷新](features/strategy-cache.md)。作者当前昵称与用户习惯写法的兼容项包括：“让我莫格鱼吧Moyu”→“让我摸个鱼吧Moyu”、“小橙子啊”→“小橙子阿”，ATRI 忽略 `丶`、`、`和反斜杠等装饰符号。
...
规则扫描时仅发现两个通用 `.*` 规则。重叠指令固定由 Auto-Plugin、miao-plugin 等其他插件优先处理，Lotus 只作末位兜底。安装新版 Auto-Plugin 后仍应重新检查默认配置和重叠规则，因为上游可能新增功能或改变默认值。

...
Lotus 作者库保存在 `data/strategy-authors/cache.json`。每天两次的后端任务 `0 10 4,16 * * ? *`、主人指令 `#更新攻略作者库` 和用户查询均使用增量检查，不再重复下载全部历史文章；当前 UTC 容器的定时任务对应北京时间 12:10 和次日 00:10。接口失败时保留本地旧缓存。完整算法、返回字段和验证命令见[三游戏攻略本地缓存与增量刷新](features/strategy-cache.md)。作者当前昵称与用户习惯写法的兼容项包括：“让我莫格鱼吧Moyu”→“让我摸个鱼吧Moyu”、“小橙子啊”→“小橙子阿”，ATRI 忽略 `丶`、`、`和反斜杠等装饰符号。

...
抽卡与个人查询同样遵守此边界：带 Lotus profile 后缀的指令始终由 Lotus 处理；无后缀指令在 `false` 时交还原插件，在 `true` 时由 Lotus 按 profile 1 处理。星铁抽卡查看虽然复用 miao 的渲染模板，但只使用可自动清理的 `data/srJson/lotus-render-*` 临时数据，不会把 Lotus 的合成记录写入 miao 正式抽卡目录。国服抽卡只读取 `account.cookie`，国际服抽卡只读取 `games.os.cookie`，避免跨区凭据误用。
...
## 与 miao-plugin 圣遗物分页的资源协调

配套 Fork 的 `#圣遗物列表` 会把每一页图片渲染登记为非签到任务。Lotus 的自动计划签到、`#签到`、`#补签` 和批量补签统一通过 `ProfileSigninService` 登记签到任务：

1. 如果一页图片已经开始渲染，该页先正常完成；
2. 等待中的签到随后取得优先权；
3. 签到结束后，miao-plugin 才继续渲染下一页；
4. 如果命令开始时签到已经在执行，第一张圣遗物图片同样等待签到结束。

协调状态只保存在当前 Yunzai 进程的 `globalThis.__LOTUS_SIGNIN_COORDINATOR__` 中，不写入配置或业务数据。只安装其中一个插件时，各自功能仍可独立运行。

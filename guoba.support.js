...
const GUOBA_SCHEMAS = [
  ...
  sw("bilibili.download.cache_enable", "启用下载缓存", "节省磁盘模式请关闭；只有需要复用已下载文件时开启，并同时关闭“发送后删除”。"),
  number("bilibili.download.cache_ttl_seconds", "缓存保留时间", "仅在启用下载缓存时生效；0 表示不按缓存时间自动过期。"),
